...
export class Bounce {
  ...
  render({ x = 0, y = 0 }) {
    const { size, track, offset, contentEl } = this._scrollbar;
    ...
    if (x) {
      ...
      const scaleRatio =
        size.container.width / (size.container.width + Math.abs(x));
    ...
    if (y) {
      ...
      const scaleRatio =
        size.container.height / (size.container.height + Math.abs(y));
